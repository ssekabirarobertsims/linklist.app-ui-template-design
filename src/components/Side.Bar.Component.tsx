...
const DashboardPageSideBarComponent: React.FunctionComponent = () => {
  const currentAdmin: (SecondaryAuthenticationProps) = React.useContext(SecondaryAuthenticationObjectContext) as (SecondaryAuthenticationProps);
    ...
    return <>
        <aside className={String("dashboard-page-side-bar-component").toLocaleLowerCase()}>
            <div>
                <ul className={String("dashboard-page-side-bar-component-ul-list").toLocaleLowerCase()}>
                    {
